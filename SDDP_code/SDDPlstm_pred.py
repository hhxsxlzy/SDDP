...
def use_tcn1(df, h, retrain):
    ...
    if retrain:
        folder_path_del = os.path.join(current_script_dir, '..', 'checkpoints', f'SDDPlstm{an}_{args.dataset}_temp')
...
def train_and_predict_tcn_for_column(h, data_column, time_index, retrain, column_name, target_column,
    ...
    model_save_dir = os.path.join(current_script_dir, '..', 'checkpoints', f'SDDPlstm{an}_{args.dataset}_temp')
    model_save_dir = os.path.abspath(model_save_dir)
    os.makedirs(model_save_dir, exist_ok=True)

    os.makedirs(model_save_dir, exist_ok=True)
    model_path = os.path.join(model_save_dir, f"lstm_{column_name}.pth")
...
class LSTMBlock(nn.Module):
    def __init__(self, in_size, hidden_size, dropout=0.2):
        """
        Parameters:
          in_size: Number of input features to this layer.
          hidden_size: Number of hidden units in the LSTM layer (i.e., output features).
          dropout: Dropout rate applied to the output of this layer.
        """
        ...
        if in_size != hidden_size:
            self.residual = nn.Linear(in_size, hidden_size)
        else:
            self.residual = None
        self.relu = nn.ReLU()
...
class LSTMPRED(nn.Module):
    def __init__(self, input_size, num_channels, dropout=0.2):
        """
        Parameters:
          input_size: Number of input features (e.g., concatenated dimension of x and other variables).
          num_channels: A list where each element specifies the output feature size of a stacked LSTMBlock.
                        The number of layers is determined by the length of the list.
          dropout: Dropout rate applied in each LSTMBlock.
        """
        super(LSTMPRED, self).__init__()
        layers = []
        num_levels = len(num_channels)
        for i in range(num_levels):
            in_size = input_size if i == 0 else num_channels[i - 1]
            hidden_size = num_channels[i]
            layers.append(LSTMBlock(in_size, hidden_size, dropout=dropout))
        self.network = nn.Sequential(*layers)
...
def lstm_predict_tplush(df, h, B, hidden_size=32, num_layers=2,
                        epochs=100, batch_size=18, learning_rate=0.001, dropout=0.2):
    """
    Train and evaluate an LSTM model using target-aware predictors.
    The data preprocessing and sliding window construction are consistent with tcn_predict_tplush.

    Parameters:
        df (pd.DataFrame): Time series DataFrame with 'date' index and 'target' column.
        h (int): Forecasting horizon (predict value at T+h).
        B (np.ndarray): Loading matrix from apply_loadings_tcn.
    
    Returns:
        rmsfe, mae (float): RMSFE and MAE metrics on the validation set.
    """
    ...
    for i in range(start_idx + 1, len(df) - h - 1):
        ...
        # Construct input window and corresponding target
    ...
    model = LSTMPRED(input_size=input_size, num_channels=[hidden_size] * num_layers, dropout=dropout)
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
...
def main():
    ...
    for i in range(k):
        ...
        # Step 3: Forecast using LSTM and compute RMSFE and MAE
        rmsfe, mae = lstm_predict_tplush(predictions_df, h=h, B=B)
        rmsfe_list.append(rmsfe)
        mae_list.append(mae)
        print(f"Iteration {i+1} - RMSFE: {rmsfe}, MAE: {mae}")

    # Compute mean RMSFE and MAE over all iterations
    ...
    output_excel_path = os.path.join(results_dir, f'results_SDDPlstm_{an}_{args.dataset}.xlsx')
