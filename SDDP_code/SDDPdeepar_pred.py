...
def use_tcn1(df, h, retrain):
    ...
    if retrain:
        folder_path_del = os.path.join(current_script_dir, '..', 'checkpoints', f'SDDPdeepar{an}_{args.dataset}_temp')
...
class DeepARTimeSeriesModel(nn.Module):
    def __init__(self, input_channels, hidden_size, num_layers, output_size=1):
        """
        Parameters:
          input_channels: Number of input features (e.g., concatenated historical targets and exogenous variables).
          hidden_size: Dimensionality of the LSTM hidden state.
          num_layers: Number of stacked LSTM layers.
          output_size: Dimensionality of the prediction output (e.g., scalar forecast of y, default is 1).
        """
        super(DeepARTimeSeriesModel, self).__init__()
...
def deepar_predict_tplush(df, h, B, hidden_size=32, num_layers=2, 
                          epochs=100, batch_size=18, learning_rate=0.001):
    """
    Train and evaluate a DeepAR model using target-aware predictors.
    The process mirrors tcn_predict_tplush, but uses DeepARTimeSeriesModel for prediction.

    Parameters:
        df (pd.DataFrame): Time series DataFrame.
        h (int): Forecasting offset (predict the value at T+h).
        B (np.ndarray): Loading matrix from apply_loadings_tcn.

    Returns:
        rmsfe, mae (float): Evaluation metrics on the validation set.
    """
    ...
    for i in range(start_idx + 1, len(df) - h - 1):
        ...
        # Normalize PCA features and target_lag
        scaler = MinMaxScaler()
    ...
    model = DeepARTimeSeriesModel(input_channels=input_size,
                                  hidden_size=hidden_size,
                                  num_layers=num_layers,
                                  output_size=1)
    criterion = nn.MSELoss()
...
def main():
    ...
    for i in range(k):
        ...
        # Step 3: Forecast using DeepAR and compute evaluation metrics
        rmsfe, mae = deepar_predict_tplush(predictions_df, h=h, B=B)
        rmsfe_list.append(rmsfe)
        mae_list.append(mae)
        print(f"Iteration {i+1} - RMSFE: {rmsfe}, MAE: {mae}")

    # Compute average RMSFE and MAE
    ...
    output_excel_path = os.path.join(results_dir, f'results_SDDPdeepar_{an}_{args.dataset}.xlsx')
